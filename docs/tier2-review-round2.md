...
### [ ] R1. Invalid-coordinate fix wipes the carried route chainage
...
### [ ] R2. Eviction runs before the `m_precomputing` early-return
...
### [ ] R3. Stale comment: `m_all` "(and Tier-1 matcher)"
...
### [ ] R4. Sustained Tier-1 fallback freezes chainage → stale window → global re-acquire
...
### [ ] R5. `kRouteReacquireMeters = 300` is a magic threshold; (150, 300] hits drop to Tier-1
...
### [ ] R6. routePath aborts the whole route on a single unroutable leg
...
### [ ] R7. Eviction can drop a still-selected train's route polyline
...
### [ ] R8. routeStations() #10 fix is incomplete (debug overlay only)
